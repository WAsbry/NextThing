...
class NotificationHelper @Inject constructor(
    ...
    companion object {
        ...
        private const val BRIEFING_CHANNEL_ID = "ai_briefing_notifications"
        private const val BRIEFING_CHANNEL_NAME = "AI 智能简报"
        private const val BRIEFING_CHANNEL_DESCRIPTION = "每日早晚的任务简报推送"
    ...
    init {
        ...
        createBriefingNotificationChannel()
    }
    ...
    private fun createBriefingNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (notificationManager.getNotificationChannel(BRIEFING_CHANNEL_ID) != null) return
            ...
            val channel = NotificationChannel(
                ...
                description = BRIEFING_CHANNEL_DESCRIPTION
                enableVibration(true)
                vibrationPattern = longArrayOf(0, 200)
                setSound(android.provider.Settings.System.DEFAULT_NOTIFICATION_URI, null)
            }
            notificationManager.createNotificationChannel(channel)
            Timber.tag(TAG).d("✅ AI 简报通知渠道已创建: $BRIEFING_CHANNEL_ID")
        }
    }

    fun showBriefingNotification(title: String, content: String) {
        val pendingIntent = context.packageManager.getLaunchIntentForPackage(context.packageName)?.let {
        ...
        Timber.tag("AI-Briefing").d("简报通知已显示: $title")
    }

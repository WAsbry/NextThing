...
class CategoryPreferencesManagerImpl @Inject constructor(
    ...
    companion object {
        ...
        private const val KEY_LAST_NOTIFICATION_STRATEGY = "last_notification_strategy"
        private const val KEY_LAST_GEOFENCE_LOCATION = "last_geofence_location"
    }

    override suspend fun getLastSelectedNotificationStrategyId(): String? {
        return sharedPreferences.getString(KEY_LAST_NOTIFICATION_STRATEGY, null)
    }

    override suspend fun saveLastSelectedNotificationStrategyId(strategyId: String?) {
        sharedPreferences.edit().apply {
            if (strategyId != null) {
                putString(KEY_LAST_NOTIFICATION_STRATEGY, strategyId)
            } else {
                remove(KEY_LAST_NOTIFICATION_STRATEGY)
            }
        }.apply()
    }

    override suspend fun getLastSelectedGeofenceLocationId(): String? {
        return sharedPreferences.getString(KEY_LAST_GEOFENCE_LOCATION, null)
    }

    override suspend fun saveLastSelectedGeofenceLocationId(locationId: String?) {
        sharedPreferences.edit().apply {
            if (locationId != null) {
                putString(KEY_LAST_GEOFENCE_LOCATION, locationId)
            } else {
                remove(KEY_LAST_GEOFENCE_LOCATION)
            }
        }.apply()
    }
}
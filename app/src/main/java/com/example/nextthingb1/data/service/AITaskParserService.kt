...
import com.example.nextthingb1.data.remote.ai.ChatCompletionRequest
import com.example.nextthingb1.data.remote.ai.ChatCompletionResponse
import com.example.nextthingb1.data.remote.ai.ChatMessage
import com.example.nextthingb1.data.remote.ai.ResponseFormat
...
class AITaskParserService @Inject constructor(
    ...
    override suspend fun parseTaskFromText(
        ...
        return try {
            ...
            Timber.tag("AI").d("🚀 请求 AI: provider=${provider.displayName}, model=$model, url=${baseUrl}chat/completions")
            ...
            Timber.tag("AI").d("✅ AI 返回 ($responseCode): ${responseBody.take(200)}")
            ...
            Result.success(parseJsonToResult(content))
...
- 日常琐事或未提及重要性 → null
    ...
    private fun parseJsonToResult(json: String): AITaskParseResult {
